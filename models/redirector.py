...
from .PerceptualSimilarity.lpips import PerceptualLoss
...
from .EyeFeatureExtractor import EyeFeatureExtractor, eyeresnet18
from .utils import crop_eye_img, feature_lerp, AdaptiveInterpolation

from .modules import VectorTransformer

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

class Redirector(nn.Module):
    def __init__(self, args):
        super(Redirector, self).__init__()
        self.configuration = []
        if config.size_0d_unit > 0:
            self.configuration += [(0, args.size_0d_unit)]
        self.configuration += ([(1, args.size_1d_unit)] * args.num_1d_units +
        ...
        self.encoder = Encoder(self.num_all_pseudo_labels, self.num_all_embedding_features, self.configuration).to(device)
        self.decoder = Decoder(self.num_all_embedding_features).to(device)
        # self.lpips = PerceptualLoss(model='lpips', net='alex').to(device)
        # self.GazeHeadNet_train = GazeHeadNet().to(device)
        # self.GazeHeadNet_eval = GazeHeadResNet().to(device)
        # self.discriminator = PatchGAN(input_nc=3).to(device)
        ...
        self.eyenet = eyeresnet18().to(device)
        self.eye_extractor = EyeFeatureExtractor().to(device)
        self.eye_extractor_optimizer = optim.Adam(self.eye_extractor.parameters(), lr=config.lr, weight_decay=config.l2_reg)
        for param in self.eyenet.parameters():
            param.requires_grad = False
        
        self.alpha_sim = torch.full((32,), 0.5, dtype=torch.float).to(device)
    ...
    def forward(self, x, infer=False):
        ...
        # Encode input from a
        embeddings_a, pseudo_labels_a = self.encoder(x)
        ...
        if 'image_b' in data:
            ...
            embeddings_b, pseudo_labels_b = self.encoder(data['image_b'])
    ...
    def optimize(self, data, current_step):
        ...
        for param in self.generator_params:
            param.requires_grad = True
        for param in self.iin.parameters():
            param.requires_grad = True
        for param in self.eye_extractor.parameters():
            param.requires_grad = True
            
        embeddings_a, pseudo_labels_a = self.encoder(data['image_a'])
        embeddings_b, pseudo_labels_b = self.encoder(data['image_b'])
        ...
        if config.use_mixing:
            ...
            decoded_embeddings_a_to_mix = self.decoder(embeddings_a_to_mix)
            embeddings_mix_hat, pseudo_labels_mix_hat = self.encoder(decoded_embeddings_a_to_mix)
        ...
        z_tr_g = self.alpha_sim * z_s_g + (1 - self.alpha_sim) * z_t_g
        g_tr = self.alpha_sim * g_s + (1 - self.alpha_sim) * g_t
        
        rotate = lambda feature, label_a, label_b: torch.matmul(self.rotation_matrix_2d(label_b, False), torch.matmul(
                        self.rotation_matrix_2d(label_a, True), feature.reshape(config.batch_size, 3, -1))).reshape(config.batch_size, -1)
        z_tr_g_tilde = rotate(z_tr_g, g_tr, g_t)
        z_t_g_tilde = rotate(z_s_g, g_s, g_t)
        
        # Consistency loss
        self.alpha_sim = 1 - F.cosine_similarity(z_tr_g_tilde, z_t_g_tilde, eps=1e-6, dim=-1)
        losses_dict['cns'] = torch.mean(1 - F.cosine_similarity(z_tr_g_tilde, z_t_g_tilde, eps=1e-6, dim=-1))
        total_loss += losses_dict['cns'] * config.coeff_cns_loss
        
        # Positive extrapolation and negative interpolation in SG loss
        z_e = self.extract_eye_feature(data['image_a']).reshape(bsz, -1)
        ...
        for pos in pos_features:
            for neg in neg_features:
                sg_count += 1
                losses_dict['sg'] += losses.triplet_loss(eye_feature, pos, neg, margin=config.contrast_margin)
        ...
        self.eye_extractor_optimizer.zero_grad() #TODO
        if config.use_apex:
            with amp.scale_loss(total_loss, [self.generator_optimizer]) as scaled_loss:
                scaled_loss.backward()
        else:
            #self.alpha.backward(retain_graph=True)
            context_similarity_loss.backward(retain_graph=True)
            total_loss.backward()
        self.generator_optimizer.step()
        self.eye_extractor_optimizer.step() #TODO
    ...
    def latent_walk(self, data):
        ...
        for j in np.linspace(-1, 1, num=walk_len, endpoint=True):
            ...
            # if j == -1: feats = embed.cpu().squeeze(0).numpy()
            # else: feats = np.concatenate((feats, temp_feat), axis=0)
        ...
        for i in range(config.num_1d_units):
            ...
            for j in np.linspace(-np.pi / 2 * 0.9, np.pi / 2 * 0.9, num=walk_len, endpoint=True):    
                angle = torch.tensor(j, dtype=torch.float).unsqueeze(0).expand(batch_size, -1).to(device)
                ...
                # if i ==0 and j == -np.pi / 2 * 0.9: feats = embed.cpu().squeeze(0).numpy().reshape(1, -1)
                # else: feats = np.concatenate((feats, temp_feat), axis=0)
        ...
        for i in range(config.num_2d_units):
            ...
            for j in walk_2d:
                ...
                # if i == 0 and idx == 0: feats = embed.cpu().squeeze(0).numpy().reshape(1, -1)
                # else: feats = np.concatenate((feats, temp_feat), axis=0)
